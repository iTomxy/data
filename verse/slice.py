...
il_path = osp.join(P, "processed-verse19")
tslab_path = osp.join(P, "processed-verse19-ts-bone-label")
save_path = osp.join(P, "processed-verse19-slice-is")

for subset in os.listdir(il_path):
    print('\t', subset)
    sub_il_p = osp.join(il_path, subset)
    sub_tslab_p = osp.join(tslab_path, subset)
    for img_f in glob.glob(osp.join(sub_il_p, "*_image.nii.gz")):
        vid = osp.basename(img_f)[: - len("_image.nii.gz")]
        print('\t', vid, end='\r')

        lab_f = osp.join(sub_il_p, f"{vid}_label.nii.gz")
        tslab_f = osp.join(sub_tslab_p, f"{vid}_label_ts.nii.gz")

        slice_p = osp.join(save_path, subset, vid)
        ...
        lab = lab.astype(np.uint8) # class id in [0, 28]
